...
def unit_piece_area(shape: tuple[int, int], contours: list[np.ndarray]) -> float:
    ...
    **连通块够多时用面积中位数。** 真实照片里绝大多数连通块就是单块碎片
    （实测一张摊得开的 50 块照片：恰好 50 个连通块，面积紧紧聚在中位数
    附近），中位数直接就是答案。
    ...
    **连通块太少时才用山峰计数归一化。** 中位数的前提是「多数连通块是单块」，
    ...
    拼图碎片的**每个凸起都是一个独立的距离变换山峰**。实测一张 50 块的
    照片上，48 块单块碎片的 peak_count 全部返回 3，没有一个是 1，单块面积被
    压低 3 倍、48/50 块碎片被判定需要切分，50 块碎片最终被劈成 96 个轮廓，
    每张裁剪图里只剩半个编号。合成图全是圆形，圆没有凸起，所以测不出来。

    两个分支的失配方向也不同，这一点很重要：面积分支在粘连多的图上会
    **低估**块数（少切），退化成「一张裁剪图里有两个编号」——这是设计上
    接受的优雅降级，功能仍在；而高估块数会把碎片切成两半，编号跟着被
    切断，是不可恢复的。所以宁可少切。
    """
    if not contours:
        return 0.0
...
def crop_piece(
    ...
    **不要改回「按自己的轮廓填灰」**（实测教训）：深色印刷字符低于阈值，
    在掩膜上是一个洞；字符紧挨拼图凹口时，那个洞会与外部背景连通，
    外轮廓便从凹口钻进碎片内部绕字符一圈，填灰时正好抹掉半个编号——
    实测把 `D-797` 抹成了 `D-79`——漏掉一位数字之后补哪一位都说得通
    （`D-790`…`D-799` 全部等距），吸附层只能判为歧义，这块就丢了。

    **也不要试图用闭运算补那道缝**：缝的口子就是凹口的口子，核大到够得着
    字符时，凹口连同邻域的深色背景一起被灌进裁剪图，实测 Pass A 命中率
    从 40/50 掉到 25/50，穷举翻倍、耗时涨到五倍。这条路是死的，
    不是参数没调好——局部形态学绕不开（背景开运算 ≡ 前景闭运算，
    同一个算子），填洞也不行（缝真的通到外面，封闭洞数为 0）。
    """
