...
def _local_ipv4_addresses() -> list[str]:
    ...
    1. psutil —— 唯一能真正遍历所有网卡的。实测：装了代理时，
       只有它能看见无线网卡的 192.168.x 地址，另外两种方法全都只返回
...
def find_free_port(port: int, attempts: int = 20) -> int:
    ...
    **判据是「连得上」而不是「绑得上」，这一点是本函数存在的全部理由。**
    ...
    实测踩过：另一个进程占着端口，服务照样「启动成功」，请求却时灵时不灵。
...
if __name__ == "__main__":
    # 端口可覆盖。别用默认的 8000：Windows 允许两个进程绑同一端口却不报错，
    # 请求会随机落到其中一个——排查起来很费劲。走 main.py 则会自动挑一个
    # 没人在听的端口。
