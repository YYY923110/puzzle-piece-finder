...
建索引耗时约 9 分钟。详见 [docs/tuning-log.md](docs/tuning-log.md)。
...
| **编号尽量朝正** | 朝正的走 Pass A（1 次识别），歪的要转 12 个角度 | 朝向随机时 35 秒/块，朝正后 11 秒/块 |
...
除两个标了 `@pytest.mark.ocr` 的测试外，所有测试注入假后端，**完全不加载 PaddleOCR**——
所以测试几秒钟跑完，也不依赖模型下载。换识别后端只需实现 `read(image) -> list[RawDetection]`。
