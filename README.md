...
**实测**（2026-08-04 最新一轮，三张实拍）：

| 照片 | 分割 | 识别率 | 耗时 |
|---|---|---|---|
| `IMG_20260805_082927.jpg`（23 块，**摆得开**） | 23/23 | **100%** | 2.65 s/块 |
| `real6.jpg`（50 块，密排） | 50/50 | **100%** | 6.72 s/块 |
| `real3.jpg`（50 块，密排） | 50/50 | 84% | 5.34 s/块 |

`real3` 的 8 块损失全部来自**同图编号重复**（见「已知限制」），不是读不出来。
碎片摆开拍不只是识别率的事，也是速度的事——摆得开的那张快一倍多，
因为几乎不用跑角度穷举。详见 [docs/tuning-log.md](docs/tuning-log.md)。
